...
/**
 * parser_helper - Receives the main string and all the necessary parameters to
...
int parser_helper(int *i1, const char *format, conver_t f_list[], va_list arg_list)
{
	...
	if (format[i] == '%') /*Checks for format specifiers*/
	{
		...
		(*(i1)) += 1; /*Updating i to skip format symbols*/
	}
	else
	{
...
int parser(const char *format, conver_t f_list[], va_list arg_list)
{
	...
	{
		printed_chars = printfFlags(&i, format, f_list, arg_list);
		printed_chars = parser_helper(&i, format, f_list, arg_list);
	}
	return (printed_chars);
}
